...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Ce ped ne l'admet pas", 3000) return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then
		...
		exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Vous n'avez pas de vêtements", 3000) return
	else
		if not LastEquipped[which] then
	...
	exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Tu n'as pas de vêtements", 3000) return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Vous n'avez rien à porter", 3000) return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Ce modele prend pas en charge cette option", 3000) return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		exports['Alpha-Notif']:Alert("warning", "Lunasty Vetement", "Il ne semble pas y avoir de variante de cela", 3000) return false
	end
end
